...
All my projects, both work and non-work, are located under the `~/src/` directory.
The directories associated with each project and the emails I wish to use for them are as follows:
...
with specific exceptions: `~/src/github.com/alexandear/**` and `~/src/github.com/epam/**`.
...
Using the config option `includeIf` and setting up related `.gitconfig` files allows you
